...
-- 1. Ensure `users` table has the required security columns
-- (If you already ran this before and it says "Duplicate column name", that is OK! Ignore the warning)
ALTER TABLE users ADD COLUMN IF NOT EXISTS is_verified BOOLEAN DEFAULT FALSE;
ALTER TABLE users ADD COLUMN IF NOT EXISTS locked_until TIMESTAMP NULL;

-- 2. Create the new OTP and Audit Logs tables cleanly
...
-- 3. Ensure existing 'admin' and 'guest' accounts use the necessary BCrypt passwords
...
-- 4. Just in case they were deleted, seamlessly re-insert them without causing duplicate errors
